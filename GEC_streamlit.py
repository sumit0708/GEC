...
class GramformerDemo:
    ...
    def main(self):
        ...
        st.title("Data pipeline/correction application")
        # st.write(f'GitHub Link - [{github_repo}]({github_repo})')
        st.markdown('Correcting grammatical errors on natural language text')
        ...
        file = st.file_uploader("Input CSV file",type ="csv")
        if file is not None:
            st.markdown("File uploaded")
            df = pd.read_csv(file)
            st.dataframe(df)
            new_df=pd.DataFrame()
            incorrect_sentence = df['content'].to_list()
            correct_sentence = []

            for x in incorrect_sentence:
                correct_sentence.append(gf.correct(x, max_candidates=max_candidates)[0])
            new_df = df
            new_df.drop('content',axis=1,inplace=True)
            new_df['corrected_sentence']= correct_sentence
            st.dataframe(new_df)
